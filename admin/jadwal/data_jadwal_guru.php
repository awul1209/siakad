						...
						<div class="card-body">
							<div class="table-responsive">
								<table id="example1" class="table table-bordered table-striped">
									...
									$no = 1;
									if ($p_hari == '') {
										$sql = $koneksi->query("SELECT guru.id_guru, jadwal.id_jadwal, jadwal.hari, jadwal.waktu, matpel.matpel, kelas.nama_kelas, guru.nama_guru FROM jadwal 
										INNER JOIN matpel ON jadwal.matpel_id=matpel.id_matpel 
										INNER JOIN kelas ON jadwal.kelas_id=kelas.id_kelas 
										INNER JOIN guru ON jadwal.guru_id=guru.id_guru 
										WHERE id_guru='$data_id'");
										...
										INNER JOIN kelas ON jadwal.kelas_id=kelas.id_kelas 
										INNER JOIN guru ON jadwal.guru_id=guru.id_guru 
										WHERE id_guru='$data_id' AND hari='$p_hari'");
									}
									while ($data= $sql->fetch_assoc()) {
										?>
