<?php
$id_jadwal = $_GET['id'];
$query = mysqli_query($koneksi,"SELECT jadwal.id_jadwal, matpel.matpel, kelas.nama_kelas FROM jadwal INNER JOIN matpel ON jadwal.matpel_id=matpel.id_matpel INNER JOIN kelas ON jadwal.kelas_id=kelas.id_kelas WHERE id_jadwal = '$id_jadwal'");
$row = mysqli_fetch_assoc($query);
?>
<div class="card card-info">
	<div class="card-header bg-gradient-dark">
		<h3 class="card-title">
		<i class="fa fa-file"></i> Input Nilai Kelas <?= $row['nama_kelas'] ?> |
	...
	<form action="" method="post">
		<div class="table-responsive">
			<div>
				<a href="?page=import-siswa" class="btn btn-primary bg-gradient-primary text-white">
					...
					if ($result->num_rows > 0) {
						while($row = $result->fetch_assoc()) {
							...
							foreach ($kriteria as $k) {
								echo "<td><input type='number' name='nilai_awal[" . $row["nis"] . "][" . $k["id_kriteria"] . "]'class='form-control'></td>";
							}
							echo "</tr>";
						}
					}
...
// var_dump($nilai);
	foreach ($nilai as $nis => $kriteria_nilai) {
		foreach ($kriteria_nilai as $kriteria_id => $nilai_siswa) {
			$sql = "INSERT INTO nilai_awal VALUES ('', '$id_jadwal','$nis','$kriteria_id','$nilai_siswa')";
